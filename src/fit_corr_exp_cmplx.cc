...
int main(int argc, char *argv[]){
  ...
  if(argc != 9){ cerr << "usage: <filename> <tmin> <tmax> <noise_cutoff> <fitCrit> <min_tslices> <covar_SV_cutoff> <real=0,imag=1>" << endl; exit(1); }
  ...
  int part; {istringstream val(argv[8]); val >> part;}
  ...
  //load the data
  EnsemVectorComplex corr_cmplx;  {ostringstream filename;   filename << filen;   read(filename.str(), corr_cmplx);}
  cout << "read file: " << filen << endl;
  int bins = peekObs(corr_cmplx, 0).size();  cout << "bins = " << bins << endl;
